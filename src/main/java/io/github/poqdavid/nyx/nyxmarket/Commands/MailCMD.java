...
import io.github.poqdavid.nyx.nyxmarket.Market.MarketMail;
...
public class MailCMD implements CommandExecutor {
    ...
    public MailCMD(Game game, Invs inv) {
        this.game = game;
        this.inv = inv;
    }

    public static Text getDescription() {
        return Text.of("Opens your mailbox");
    }
    ...
    public static CommandElement getArgs() {
        return GenericArguments.optional(GenericArguments.playerOrSource(Text.of("player")));
    }
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (src instanceof Player) {
            ...
            if (src.hasPermission(MarketPermission.COMMAND_BUY)) {
                final MarketMail marketmail = new MarketMail(player_cmd_src);
                player_cmd_src.openInventory(marketmail.getmarketmail());
            } else {
