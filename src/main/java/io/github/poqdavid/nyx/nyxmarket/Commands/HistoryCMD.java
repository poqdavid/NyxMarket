...
public class HistoryCMD implements CommandExecutor {
    ...
    public HistoryCMD(Game game, Invs inv) {
        this.game = game;
        this.inv = inv;
    }

    public static Text getDescription() {
        return Text.of("Shows past transactions");
    }
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (src.hasPermission(MarketPermission.COMMAND_HISTORY)) {
            final String search = args.<String>getOne("search").orElse("*");
            Tools.SendHistory(src, search);
